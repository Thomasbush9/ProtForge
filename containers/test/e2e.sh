...
snakemake --profile profiles/slurm/ --configfile "$CONFIG" -n
...
Or invoke snakemake directly (output WILL be appended to ${E2E_LOG}):
  cd ${REPO_ROOT}
  snakemake --profile profiles/slurm/ --configfile ${CONFIG} 2>&1 | tee -a ${E2E_LOG}
...
set +e
snakemake --profile profiles/slurm/ --configfile "$CONFIG"
RC=$?
set -e
trap - INT
...
if [[ $RC -eq 0 ]]; then
    ...
    echo "FAILED. Inspect:"
    echo "  ${E2E_LOG}              (this script + snakemake stderr/stdout)"
    echo "  ${LOG_DIR}/*.log        (per-rule SLURM logs)"
fi
