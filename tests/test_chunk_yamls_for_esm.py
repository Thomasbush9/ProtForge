...
from chunk_yamls_for_esm import create_chunks, find_yaml_files
...
class TestCreateChunks:
    def test_naming(self, tmp_path):
        ...
        create_chunks(files, str(output_dir), num_chunks=2)
    ...
    def test_caps_num(self, tmp_path):
        ...
        chunk_files = create_chunks(files, str(output_dir), num_chunks=10)
    ...
    def test_content_absolute_paths(self, tmp_path):
        ...
        chunk_files = create_chunks(files, str(output_dir), num_chunks=1)
    ...
    def test_manifest(self, tmp_path):
        """manifest.txt lists all chunk files."""
        yaml_dir = tmp_path / "yamls"
        yaml_dir.mkdir()
        files = [make_yaml(yaml_dir, f"seq_{i}", "MKTL") for i in range(5)]
        output_dir = tmp_path / "output"

        chunk_files = create_chunks(files, str(output_dir), num_chunks=3)

        manifest = (output_dir / "manifest.txt").read_text().strip().splitlines()
        assert len(manifest) == len(chunk_files)
