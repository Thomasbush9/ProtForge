...
# ===================== Mode: Custom =================================
    ...
    if [[ -d "$ESM_ENV" ]]; then
        ...
        "$ESM_ENV/bin/pip" install esm torch numpy
...
if command -v snakemake &>/dev/null; then
    ...
    echo "  [!!] Snakemake not found — pip install snakemake snakemake-executor-plugin-slurm"
    ERRORS=$((ERRORS + 1))
fi
...
if [[ $ERRORS -eq 0 ]]; then
    ...
    echo "  2. Place your input FASTAs in: $FASTA_DIR"
    echo "  3. Dry run:  snakemake --profile profiles/slurm/ -n"
    echo "  4. Launch:   snakemake --profile profiles/slurm/"
else
    echo "=============================================="
